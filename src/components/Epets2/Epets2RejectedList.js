...
import Epets2RejectedCard from "./Epets2RejectedCard";
...
const RejectedList = (props) => {
    ...
    const getForms = () => {
        setLoading(true)
        axios.get(process.env.REACT_APP_API_URL+'/epet2-forms-rejected/',{headers:{'Authorization':'Token '+token}})
            .then(res=>{
                setForms(res.data)
                setLoading(false)
            })
