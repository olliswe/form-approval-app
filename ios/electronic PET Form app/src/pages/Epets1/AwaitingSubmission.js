...
const AwaitingSubmission = (props) => {
    ...
    const getForms = () => {
        setLoading(true)
        axios.get(process.env.REACT_APP_API_URL+'/forms-awaiting-submission/',{headers:{'Authorization':'Token '+token}})
            .then(res=>{
                setForms(res.data)
                setLoading(false)
            })
            ...
            <IonContent>
                ...
                <IonGrid>
                    <IonRow className='ion-justify-content-center ion-margin-horizontal'>
                        <h2>EPET-1 Forms awaiting submission to MoF by Vote Controller</h2>
                    </IonRow>
                </IonGrid>
                {loading ?
                    <IonList>
                        ...
                        forms.length>0 ?
                            ...
                            <IonGrid>
                                <IonRow className='ion-margin ion-justify-content-center'>
                                    No forms awaiting submission to MoF
                                </IonRow>
                            </IonGrid>
                    )
...
export default withRouter(AwaitingSubmission)